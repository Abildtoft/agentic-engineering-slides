...
| `SoftwareMiddleDiagram`, `KitchenAnalogyDiagram`, `GuardrailsLaneDiagram`, `SwissCheeseDiagram`, `ConfidenceSpiralDiagram`, `CommitGrowthChart` | Redrawn from the old raster illustrations as brand-token SVG (theme-aware via `var(--brand-*)`, no per-theme image pairs needed). CSS-only staggered rise entrances on the motion tokens, line draws via `pathLength="1"` dashes; hidden states live in keyframe `from`s. Arrowheads on drawn lines are polygons that fade in when the draw lands, never `marker-end` — a marker sits at the path's end from the start, ignoring the dash draw. `SwissCheeseDiagram` *reads* `$clicks` without consuming any: its "incident" through-arrow rides the slide's second text click as a transition (reversible, instant backward), brightening the aligned holes as it threads them. `CommitGrowthChart` is the exception on entrances: its only call site sits inside a `<v-click>`, and a slide-entry animation would run to completion behind the opacity-hidden wrapper — so its draw rides the wrapping v-click state as a transition (`:global(.slidev-vclick-hidden)` holds the undrawn state), playing when the click reveals it |
